...
testGeneNumber <- 3 ### 所有gene计算需要花费的时间较多，可以先选择部分gene 进行测试运
### 如果需要这个参数产生效应，需要将下面这列代码生效：#gene_name_list_test <- gene_name_list[1:testGeneNumber]
library(tidyverse)
library(Biostrings)
...
df <- gene_match_fun(gene_name_list_test,ncbi_GFF_clean) # gene name match transcript_id
...
######### save files ####
# save(ncbi_vcf_clean,file = paste0(output_path,"ncbi/","ncbi_vcf_clean.RData"))
# save(ncbi_GFF_clean,file = paste0(output_path,"ncbi/","ncbi_GFF_clean.RData"))
# save(ncbi_mrna_clean,file = paste0(output_path,"ncbi/","ncbi_mrna_clean.RData"))
save(df.4.3,file = paste0(output_path,"test/","ncbi_final_df.4.3.RData"))
save(df.5.5,file = paste0(output_path,"test/","ncbi_final_df.5.5.RData"))
#### Reference ####
#1 关于GFF的格式说明，见如下链接：
...
time.list <- list(time.df.5.2 = time.df.5.2,
                  ...
                  time.all = time.all)
save(time.list,file = paste0(output_path,"test/","time.record.RData"))
