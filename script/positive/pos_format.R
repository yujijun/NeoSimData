...
library(tidyverse)
pos.mutate <- readr::read_delim(file = "./output/pos/pos.fraction.mutate.fsa.out")
pos.wild <- readr::read_delim(file = "./output/pos/pos.fraction.wild.fsa.out")
all <- pos.wild %>% 
  left_join(pos.mutate,by = c("seq_num" = "seq_num")) %>% 
  select(seq_num,peptide.x,peptide.y,allele.x,ic50.x,rank.x,ic50.y,rank.y) %>% 
  add_count(seq_num) %>% 
  filter(n == 1) %>% 
  select(!seq_num) %>% 
  select(!n)
colnames(all) <- c("wild_peptide","mutate_peptide","allele",
                   "wild_ic50","wild_rank","mutate_ic50","mutate_rank")
write.table(all,file = "./output/pos/final.pos.neoantigens.filtered.tsv",quote = F,
...
# 
# pos.neoantigens <- cbind(
#   reformat.fraction.wild,
...
# 
# write.table(pos.neoantigens.filtered,
#   file = paste0(output_path, "final.pos.neoantigens.filtered.tsv"),
